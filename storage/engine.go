...
func (e *Engine) WritePoints(ctx context.Context, points []models.Point) error {
	...
	for iter := collection.Iterator(); iter.Next(); {
		...
		// Drop any series with invalid unicode characters in any of the tag keys or values.
		...
		if !bytes.Equal(tags[0].Key, tsdb.MeasurementInternalTagKeyBytes) {
		...
		if !bytes.Equal(fkey, tsdb.FieldKeyInternalTagKeyBytes) {
